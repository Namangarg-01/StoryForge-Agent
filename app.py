...
#Select the Model 
MODEL_INFO = "gemini-2.0-flash" #For getting real time info
MODEL_SCRIPT = "gemini-2.0-flash" #For content creation ( Can go for even better models)
...
def get_realtime_info(query): # Passing query to tavily API for getting real time info and furture passing this info to content creation 
    try:
        resp = tavily.search(
            query=query,
            max_results=3, # Getting top 3 results
            topic="general"
        )
    ...
    """
    try: 
        response = client.models.generate_content(contents=prompt, model=MODEL_INFO)
...
def generate_video_script(info_text):
    ...
    """
    try:
        response = client.models.generate_content(contents=prompt, model=MODEL_SCRIPT)
        return response.text if response and response.text else "Could not generate video script"
    except Exception as e:
        ...
        if info_result:
            ...
            if generate_script == "Yes":
                ...
                with st.spinner("🎬 Crafting your script..."):
                    ...
                    # st.code(script, language="markdown") #Different way of writing markdown
